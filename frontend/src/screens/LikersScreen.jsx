...
export function LikersScreen() {
  ...
  useEffect(() => {
    postsApi.likesList(postId).then(r => { setUsers(r.data.users||[]); setLoading(false); }).catch(() => setLoading(false));
  }, [postId]);
  ...
  return (
    <div>
      ...
      {loading
        ...
        : users.length === 0
          ? <div className="empty-state"><div className="icon">❤️</div><div className="title">No likes yet</div><div className="subtitle">Be the first to like this post!</div></div>
          : users.map(u => (
